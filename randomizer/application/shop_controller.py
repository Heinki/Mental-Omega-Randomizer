...
class ShopController(ShopPolishController):
    def initialize_shop_controller(self):
        ...
        self.shop_permanent_power_filter_var = tk.StringVar(value='All')
    ...
    def give_up_shop_run(self):
        ...
        if not messagebox.askyesno(
            ...
            'Permanent Gems and permanent unlocks are kept.',
            parent=self,
        ):
            return
        try:
    ...
    def start_shop_run(self):
        ...
        if not self.config.get('shop_mode_rules_acknowledged', False):
            if not messagebox.askokcancel(
                ...
                'one attempt.\n\nDo not save, load, or restart a mission. Any '
    ...
    def _refresh_permanent_shop(self):
        ...
        power_filter = self.shop_permanent_power_filter_var.get()
        power_entries = sorted(
            (
                ...
                if (
                    ...
                    or (
                        power_filter == 'Owned'
                        and entry.reward_id in owned_powers
                    )
                    or (
                        power_filter == 'Not Owned'
                        and entry.reward_id not in owned_powers
                    )
                )
                and (
                    not term or term in (
