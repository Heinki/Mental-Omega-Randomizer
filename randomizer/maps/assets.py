...
GENERATED_MUTATION_RULES_MARKER = (
    '; Generated by Mental Omega Randomizer Launcher mutation preload'
)
...
def _art_cameo_sections(text, preserve_keys=False):
    ...
    for raw_line in text.splitlines():
        ...
        if current and line and not line.startswith(';') and '=' in line:
            ...
            parsed_key = key.strip() if preserve_keys else key.strip().lower()
            sections[current][parsed_key] = (
...
def _stage_mutation_preload_rules(aliases, target_dir, rules_cache_path=None):
    ...
    if not source.is_file():
        raise CustomAssetError(f'Installed rules cache is missing: {source}')
    text = source.read_text(encoding='utf-8', errors='ignore')
    registry = _art_cameo_sections(text).get('ANIMATIONS', {})
    if not registry:
        raise CustomAssetError('Installed rules contain no animation registry')
    registered = {str(value).upper() for value in registry.values()}
    next_key = max((int(key) for key in registry if str(key).isdigit()), default=-1) + 1
    additions = {}
    for animation in animations:
        if animation.upper() not in registered:
    ...
    if target.exists() and not target.read_text(
        encoding='utf-8', errors='ignore'
    ).startswith(GENERATED_MUTATION_RULES_MARKER):
...
def deploy_generated_unit_art(
    ...
    if staged:
        deployed = _activate_runtime_assets(
            mutation_assets + [target] + _requested_runtime_asset_paths() + unit_assets
        )
