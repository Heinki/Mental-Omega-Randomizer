...
def enemy_effect_text(reward, count=1):
    """Describe cumulative effect for UI and reward text."""
    ...
    if effect == 'armor':
        received = stacking_multiplier('armor', count)
        amount = int(round(((1.0 / received) - 1.0) * 100))
        return f'{category} armor +{amount}%'
    if effect == 'production':
        amount = int(round(
            (1.0 - stacking_multiplier('production', count)) * 100
        ))
        return f'{category} production time -{amount}%'
    if effect == 'power':
...
def enemy_reward_display_name(reward, count=1):
    return f'AI Reward: {enemy_effect_text(reward, count)}'
