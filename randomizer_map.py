...
def stacked_house_buff_values(rewards, base_values=None):
...
def unit_weapon_buff_rules(lines, rewards, buff_allied_helpers=False):
    ...
    allowed_names = []
    for house in [player_house] + helper_houses:
...
def catalogue_tech_ids():
    if not NO_BASES_INI.exists():
        return set()

    lines = read_text(NO_BASES_INI).splitlines()
    tech_ids = set()
    ignored = {'CNTR', 'AMCV', 'SMCV', 'PCV', 'FMCV'}
    current = None
    current_values = {}

    def flush():
        if not current or current in ignored:
            return
        allowed = current_values.get('allowedtostartinmultiplayer', '').lower() == 'yes'
        goodie = current_values.get('crategoodie', '').lower() == 'yes'
        if allowed or goodie:
            tech_ids.add(current)
...
def controlled_tech_ids():
    ...
    tech_ids.update(catalogue_tech_ids())
...
def clone_player_country_for_house_buffs(lines, rewards):
    player_house = player_house_from_map(lines)
    if not player_house:
        return ('', {})
    ...
    house_buff_values = stacked_house_buff_values(rewards, original_values)
...
def allied_helper_houses(lines, player_house):
    ...
    for name, record in records.items():
        ...
        if not is_buffable_helper_house(record):
            continue
        if not same_helper_family(player_record, record):
            continue
        helpers.append(name)
...
def player_country_buff_rules(lines, rewards, allow_shared_country=False, buff_allied_helpers=False):
    player_house = player_house_from_map(lines)
    if not player_house:
        return ('', '', {}, [], [], [])
    ...
    helper_houses = allied_helper_houses(lines, player_house) if buff_allied_helpers else []
    allowed_houses = unique_in_order([player_house] + helper_houses)
    ...
    if buff_allied_helpers:
        ...
        for helper in helper_houses:
            ...
            helper_buff_values = stacked_house_buff_values(rewards, original_values)
