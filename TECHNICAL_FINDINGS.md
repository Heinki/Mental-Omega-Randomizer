...
Each stack uses normal house/category multipliers for production time x`0.85`, cost x`0.80`, and incoming armor damage x`0.90`. Production has a 10-stack safety cap (`0.85^10`, serialized as `0.197`): deterministic planning stops assigning it at that point, the UI reports only the effective count, and map generation clamps legacy excess stacks before calculating either country multipliers or direct clone `BuildTimeMultiplier`. This prevents large legacy states from serializing production as zero and driving helper AI queues into pathological instant-production loops. Movement speed is always a guarded direct TechnoType value: x`1.10` until the unit reaches its reviewed ceiling (infantry `8`, VehicleTypes/naval `12`, AircraftTypes `30`). Faster authored identities retain their base speed without further acceleration. Accessible unit types also receive guarded health and weapon-damage x`1.15`, per-unit weapon fire delay x`0.90`, and `0.5` cells of weapon range per stack. Values compound with earned buffs. Mission Details and the compact Retry Assistance block in Unlocks show the selected mission's current stack count and cumulative effects in player-facing language such as "higher", "faster", "cheaper", and "damage taken lower" rather than raw signed multipliers.
...
native. This does not change starting/Tier-1 clone access or stolen/captured
production access to the earned clone.
