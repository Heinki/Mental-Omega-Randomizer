...
If map extraction or hook preparation throws, the launcher logs the traceback, cleans generated root maps, and can still start the mission without automatic objective detection. If no victory candidate exists, the mission remains playable but automatic victory progress is unavailable; the hidden debug completion control is the recovery path. Shop has no shared mission-tree selection, so its log-only recovery control includes a picker populated from the current mission choices. Completing an uncommitted choice first selects and commits that exact mission, then uses the normal idempotent Shop victory transaction and stage advancement.
...
MCV production is part of the access pool, with separate Allied, Soviet, Epsilon, and Foehn unlocks behind their War Factories. Map-placed and script-created MCVs remain intact, so campaign openings cannot be progression-locked. Miners, Engineers, amphibious transports (Voyager, Zubr, Mandjet, and Watercat), refineries, core production, and other base-operation essentials remain outside the access pool. Generated maps force each transport to TechLevel 1 behind its matching faction shipyard. Chaos accepts any faction shipyard. Earned access removes launcher locks and is forced to TechLevel 1 in future generated maps.
...
The optional Tier 1 starter roster models seven roles: ground infantry, anti-air infantry, ground tank, anti-air tank, basic aircraft, naval attack, and naval anti-air. Standard seed-selects one concrete identity for every role and every usable faction family on isolated `<seed>:starting-tier-one-standard:<family>` streams. Allied and Soviet each store seven identities; Epsilon stores six because `SLED` fills both naval roles. Each launch prepares every physical, scripted, or potentially capturable production family. All entries retain exact Barracks/War Factory/Airfield/Naval Yard prerequisites, so a foreign starter remains dormant until its matching factory is acquired. Legacy Standard role-marker or oversized subtype saves deterministically normalize to the current per-family selection at runtime. Ground tank candidates include Bulldog/Cavalier/Kappa, Rhino/Qilin/Jaguar, and Lasher/Mantis/Opus; anti-air tank candidates include Stryker/Archon/Tsurugi, Tigr/Halftrack, and Gatling Tank. Aircraft candidates include Stormchild/Harrier/Black Eagle, Foxtrot, and Dybbuk-Attacker. One candidate fills each role; siblings are not granted. Chaos and Shop preserve separate exact mixed seeded rosters. Elite Force chooses one infantry and one vehicle removal from an isolated seed stream, rejecting the pair that would remove both anti-air starters.
...
Gear Change remains independent from Industrial Plant access. Its private
`MORGearChange` copy is granted through action `34`, ignores foreign-tech gates,
and uses an invisible exact-House provider. The Hunter-Seeker payload permits
every player country and targets shared `fact` armor, disabling Allied, Soviet,
Epsilon, and Foehn War Factories without changing native mission power types.
