...
- Mercury, Devourer, and Nanofiber Sync are EMPulse source-dependent. Each copied power uses four hidden direct-fire startup cannons with `Owner`/`RequiredHouses` set to the actual grant country and target range `0..9999`. Mercury retains its complete private laser chain; Devourer uses a private zero-minimum-range weapon chain; Nanofiber uses its installed seven-stage chain. Wallbuster instead uses its registered private `MultiMissile` chain and needs no startup object. Map-start chunks retain the 16-action ceiling and split earlier when a serialized line would exceed 511 bytes.
...
- A 97-map maximum-power/veterancy audit produced all three groups of four hidden EMPulse cannons on every map, direct-fire references for Mercury, Devourer, and Nanofiber, the complete registered Wallbuster `MultiMissile` chain without startup cannons, no missing/unknown Academy IDs, and compact Elite Reserves targets on every map.
- Power buffs are separate rewards and settings from unit/building buffs. `rewards/power_buffs.json` explicitly groups safe applicability for recharge, activation cost, effect area, direct damage, timed duration, and delivered payload count. Planning requires the matching power access first. Launch folds stacks into the canonical reward copy before `cloned_superweapon_plan`; a runtime-canonical marker prevents downstream compatibility canonicalization from discarding those overlays. Warhead area/duration changes use private auxiliary clones. A 97-map maximum-stack audit produced 7,760 power clones, zero missing dependencies/native power edits/type-ID overflows, and six grant-action chunks per map with a 502-byte maximum.
