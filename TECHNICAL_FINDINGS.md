...
The independent Tier 1 defense starter stores one abstract marker without consuming RNG. Standard resolves it from physical player/scripted Construction Yard families: Allied `GAPILL`/`NASAM`, Soviet `NALASR`/`NAFLAK`, and Epsilon `YAGGUN`, each behind only its native Yard. Native Foehn `FASONI`/`FAGUAR` remains Chaos-only. Chaos exposes all seven through Ares alternatives for `GACNST`, `NACNST`, `YACNST`, and `FACNST`. No discovered player base construction means no injected defense rule. Advanced Pool exclusions override starter injection and reward-pool suppression. Eligible starter defense access rewards are removed while defense buffs remain eligible immediately. The marker adds no RNG calls, preserving existing seed streams.
...
Chaos Tier 1 defenses use no random stream: all configured faction defenses are eligible whenever any player Construction Yard exists. Each resulting rule accepts all four Yard families, while exclusions still win.
