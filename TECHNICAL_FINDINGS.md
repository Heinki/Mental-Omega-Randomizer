...
4. Apply safe map-local country/house buffs to player-controlled houses. Allied-helper reward injection is currently suspended so native helper AI remains untouched.
...
The assistance is written only into the generated copy of the selected mission map. Campaign trigger owners are country IDs, so player and helper houses remain on their original countries. Country/category multipliers are applied only when every active house in that country family is safely player-controlled; otherwise that country-level part is skipped and logged. AI-helper assistance and reward injection are suspended, regardless of the retained seed option. The reviewed mission allowlist remains available for native-production protection and future work. Health uses global TechnoType fields and damage/range use global WeaponType fields, so those bonuses pass through the map-usage guard. Every enemy/helper-shared direct unit or weapon field is skipped unless a player-only clone isolates it. No global INI or MIX archive is changed.
...
House-supported rewards use map-local country data for production time, construction time, category cost, category speed, category armor, army ROF, and veteran lists. Only player-controlled houses participate. `buff_allied_helpers` remains seed-compatible but is currently inert.
...
Unit-access `Owner` and `RequiredHouses` values are narrowed at map launch from broad faction lists to the current player countries plus safely isolated configured helper countries. A helper country is omitted when a denied active house shares or inherits it. The engine gates production by Country/HouseType rather than the runtime campaign House instance: if a denied AI and the player share the same Country, access ownership cannot distinguish them without reassigning the player's Country and breaking country-owned mission triggers. Player access is preserved in that edge case; unsafe country/category and direct unit/weapon effects remain skipped.
...
Unsafe raw type changes are never written onto an enemy/helper-shared original. Mandatory narrow player clones isolate supported direct TechnoType and WeaponType effects. Unsupported indirect/spawned paths remain skipped and logged.
...
Narrow player unit cloning is mandatory launch behavior, not a setting and not seed-frozen. Legacy `generation.experimental_player_unit_clones` values are ignored and removed from newly saved settings. Clones are created only for currently relevant direct buffs or player-access types shared with native helper production. The first live run proved Ares `$Inherits` unsafe here: `[MORWSCARHALFTRACKGUNX]` contained only buff overrides, so WeaponType construction failed because inherited `Projectile=InvisibleWork` was unavailable. Narrow clones contain complete installed values plus map-local overrides, with `$Inherits` removed. A clone whose original omits `Image=` receives `Image=<original ID>` because the engine would otherwise seek art under the new `MORP...` ID. Only player placements and exclusively player-owned TaskForces are rewritten; every AI reference remains original.

The first successful isolation tests kept enemy Conscripts and Soviet 12 Flak Troopers unbuffed. Later `FBEYOND` tests exposed two production traps. Forcing `BuildLimit=-1` onto every clone made it a permanent one-build allowance: the cameo vanished after the first completed unit, so repeatable clones omit generated `BuildLimit`. Conversely, forcing `BuildLimit=0` onto originals hid duplicate player cameos but prevented helper Autocreate teams from assembling. `Owner` and `ForbiddenHouses` alone did not hide the original cameo. Replaced originals now always use `TechLevel=11`, no launcher BuildLimit, and native AI prerequisites/ownership, including types whose installed owners are entirely player-side. This hides them from human production while keeping TaskForce production; 88 installed campaign maps already use 520 Autocreate map/type combinations at TechLevel 11 without BuildLimit 0. A maximum-pressure 97-map audit generated 4,437 clone cases, hid all 4,414 buildable originals, preserved 543 helper TaskForces, and found no failures. A type without a supported player clone stays shared and unbuffed rather than sacrificing either player's access or helper production.
...
Helper clone and roster-substitution experiments are removed. They stopped native attack production and leaked Rocketeers/Epsilon Elites into Standard player rosters. No `MORAI...` type is emitted, no helper TaskForce is copied or rewritten, and no helper unit is replaced with a player-unlocked type. Helper Autocreate source IDs are used only to restore native production rules after global access locks. Player clone eligibility comes only from actual mission access rules rather than raw seed unlock IDs; global randomized ownership is player-only. A 30-helper-production-map audit verified 543 Autocreate TaskForces unchanged, 344 helper source IDs restored without launcher locks, and 290 player clones free of helper ownership.

Country veteran lists require exact TechnoType IDs. `VeteranUnits=ABRM` does not affect a hypothetical `MORPABRM`; the original ID is replaced only in player countries. Appending every clone made `VeteranUnits` reach 755 characters, which Phobos rejected as `VeteranUnits=M`; generated values stay below 500 characters. Helper countries and their native lists remain untouched. Indirect payload/fragment weapons and spawned-missile damage stored under `[General]` remain on the guarded global path. Runtime validation of restored helper attacks, loading cost, save/load behavior, and mission-script compatibility remains required.
