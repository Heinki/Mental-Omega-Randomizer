...
| Start with basic Tier 1 combat units | `generation.start_with_tier_one_units` | `false` | Grants ground and anti-air infantry, ground and anti-air vehicles, plus a basic aircraft from seed start. Advanced Pool exclusions override these starters, so an excluded Tier 1 unit is not silently granted by this option. Selected single-campaign Standard translates the remaining roles to each matching physical Allied, Soviet, or Epsilon production family, including factories or MCVs captured later; exact prerequisites keep them dormant until capture. Captured MCV/Construction Yard families also unlock their airfield. Foehn Standard uses Allied/Soviet operating technology; native Foehn starters remain Chaos-only. All Campaigns and Chaos keep exact-ID unlock policy. Starter access rewards are removed from that seed's reward pool, but their buff rewards remain eligible immediately. Summary displays every concrete starter variant with its cameo instead of abstract role-marker names. |
| Start with basic Tier 1 defensive structures | `generation.start_with_tier_one_defenses` | `false` | Grants each available Construction Yard family's basic ground and anti-air defense: Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, and Epsilon Gatling Cannon. Selected single-campaign Standard also prepares matching defenses behind placed or scripted MCVs/Construction Yards captured later; exact Yard prerequisites keep them dormant until capture. Chaos grants all seven faction defenses behind any available Construction Yard, adding Foehn Sonic Emitter/Shrike Nest. Advanced Pool exclusions override starters. When defensive-building rewards are enabled, their access rewards leave the seed pool while buffs remain eligible immediately. |
...
With **Start with basic Tier 1 combat units**, Standard immediately grants five roles: basic ground infantry, anti-air infantry, a basic ground vehicle, an anti-air vehicle, and a basic aircraft. Their faction follows physical production; their vehicle and aircraft variant follows the player house's subfaction. United States therefore receives Bulldog/Stryker/Stormchild, Europeans Cavalier/Archon/Harrier, Pacific Kappa/Tsurugi/Black Eagle, Russia Rhino/Tigr/Foxtrot, China Qilin/Halftrack/Foxtrot, Latin Jaguar/Halftrack/Foxtrot, PsiCorps Lasher/Gatling Tank/Dybbuk-Attacker, Scorpion Cell Mantis/Gatling Tank/Dybbuk-Attacker, and Epsilon HQ Opus/Gatling Tank/Dybbuk-Attacker. Installed IDs are `MTNK` for Cavalier, `STORM` for Stormchild, and `FOX` for Foxtrot. In selected single-campaign Standard, every placed or scripted MCV, Construction Yard, and factory prepares the corresponding T1 roles behind that exact physical prerequisite; foreign units, defenses, and airfields remain dormant until actual capture. **All Campaigns** preserves exact-ID access, and Chaos preserves its exact seeded roster instead of translating captured production. Foehn Standard still uses Allied/Soviet operating technology.

With **Start with basic Tier 1 defensive structures**, Standard grants Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, or Epsilon Gatling Cannon when the matching physical Construction Yard family is available or capturable in a selected single campaign. Every structure keeps its native Construction Yard prerequisite, so the cameo stays unavailable until the MCV or Yard is actually captured. Native Foehn defenses remain Chaos-only. Advanced Pool exclusions always win.
