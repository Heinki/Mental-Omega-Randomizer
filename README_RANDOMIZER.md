...
| `speed` | Movement speed | 10% faster until its cap | Vehicles and aircraft retain the existing house/category or unit-specific behavior. Infantry uses an isolated direct clone: ground infantry cannot exceed Speed `10`, the installed Chitzkoi/Libra maximum; InfantryTypes already at or above that ceiling are omitted from the Mobility reward pool. Faster airborne InfantryTypes keep their native speed. |
