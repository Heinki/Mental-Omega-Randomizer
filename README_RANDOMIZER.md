...
| Start with basic Tier 1 combat units | `generation.start_with_tier_one_units` | `false` | Grants ground and anti-air infantry, ground and anti-air vehicles, plus a basic aircraft from seed start. Advanced Pool exclusions override these starters. Every mode resolves each role to one equivalent for every available Allied, Soviet, Epsilon, or Foehn production family; each equivalent retains its own exact factory prerequisite, so captured foreign technology remains dormant until that faction's factory exists. Captured MCV/Construction Yard families similarly prepare their own airfield. Starter access rewards are removed from that seed's reward pool, but their buff rewards remain eligible immediately. Summary displays every concrete starter variant with its cameo. |
| Start with basic Tier 1 defensive structures | `generation.start_with_tier_one_defenses` | `false` | Grants each available Construction Yard family's basic ground and anti-air defense: Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, Epsilon Gatling Cannon, and Foehn Sonic Emitter/Shrike Nest when Foehn production is available. Standard and Chaos keep every defense behind its own faction Construction Yard, so foreign defenses remain dormant until capture. Advanced Pool exclusions override starters. When defensive-building rewards are enabled, their access rewards leave the seed pool while buffs remain eligible immediately. |
...
With **Start with basic Tier 1 combat units**, Standard immediately grants five roles: basic ground infantry, anti-air infantry, a basic ground vehicle, an anti-air vehicle, and a basic aircraft. Their concrete variant follows physical production and the player house's subfaction. United States therefore receives Bulldog/Stryker/Stormchild, Europeans Cavalier/Archon/Harrier, Pacific Kappa/Tsurugi/Black Eagle, Russia Rhino/Tigr/Foxtrot, China Qilin/Halftrack/Foxtrot, Latin Jaguar/Halftrack/Foxtrot, PsiCorps Lasher/Gatling Tank/Dybbuk-Attacker, Scorpion Cell Mantis/Gatling Tank/Dybbuk-Attacker, and Epsilon HQ Opus/Gatling Tank/Dybbuk-Attacker. Installed IDs are `MTNK` for Cavalier, `STORM` for Stormchild, and `FOX` for Foxtrot. Every placed or scripted faction factory prepares exactly one corresponding role behind its own prerequisite; foreign units and airfields remain dormant until actual capture. Chaos follows the same exact-factory rule.

With **Start with basic Tier 1 defensive structures**, Standard and Chaos grant the matching Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, Epsilon Gatling Cannon, or Foehn Sonic Emitter/Shrike Nest only when that physical Construction Yard family is available or capturable. Every structure keeps its own native Yard prerequisite, so its cameo stays unavailable until the matching MCV or Yard is actually owned. Advanced Pool exclusions always win.
...
Chaos draws access and buffs independently from all four factions. One earned role prepares one equivalent per faction, but each equivalent requires its own faction's barracks, War Factory, airfield, shipyard, or Construction Yard. Capturing another faction's production building activates only that already-earned faction mapping; it never grants a reward or lets one faction's factory build another faction's unit. Special map barracks and Stalin's Fist retain their reviewed mission-specific behavior. Starter aircraft, units, and defenses use the same exact-faction prerequisites.
...
|---|---|
| Allies | Airborne, Bloodhounds, Lightning Rod, Ultra Miner, Kingsnakes, Paladin Aid, Force Shield, Target Painter, Sonar Pulse, Mercury Strike, Satellite Scan, Black Widow Alpha, Black Widow, Chronoboost, Cryoshot, Cryospear, Glacial Screen, Cryomine Field, Chronolift |
