...
2. Put `MentalOmegaRandomizer.exe` and `RandomizerLauncherRuntime` in the Mental Omega game folder, next to `MentalOmegaClient.exe`, `Syringe.exe`, and `gamemd.exe`.
...
`MentalOmegaRandomizer.exe` is an on-directory build. Players need the EXE and its adjacent `RandomizerLauncherRuntime` folder in the Mental Omega game folder; Python and the source folder are not required. Keeping the runtime files extracted avoids the slow startup of a one-file build.
...
That packages the Python launcher and Tkinter into `MentalOmegaRandomizer.exe` plus `RandomizerLauncherRuntime` in the game folder. Install PyInstaller first with `python -m pip install pyinstaller`.
...
- Adds an `Unlocks` tab that groups earned access rewards and buffs by faction and displays each unit's real in-game cameo. Shared Chaos buff groups and Standard Foehn Allied/Soviet bundles place their cameos side by side with names and combined bonuses. Cameo PCX files are extracted from the installed MIX archives, decoded without an external Python image package, and cached under `RandomizerLauncherData\cameo_cache` in packaged mode.
...
- All unearned enabled access targets receive `BuildLimit=0`; regular units also retain the TechLevel sentinel. Defensive buildings participate only when `include_defensive_buildings` is enabled. Earned access removes both restrictions on the next mission launch. Script-critical units rely on the safer build limit so campaign-created teams and preplaced units remain usable.
...
- `All Campaigns` uses the combined four-faction reward pool, keeps every unit buff independent, and retains a basic mixed-production safety net. Selecting one campaign keeps rewards strictly on that faction and translates earned access into equivalent units for off-faction barracks, factories, air commands, and shipyards found in mixed missions. Both placed factories and numbered House-section base plans are scanned; the latter is required for Epsilon 07's captured Soviet/Chinese base. The translated rule includes the physical Allied/Soviet factory prerequisite and the active player country, so captured production works for Epsilon/Foehn houses instead of changing only TechLevel. Equivalent infantry, vehicles/tanks, aircraft, naval units, transports, support units, capital ships, and defenses inherit compatible buffs; genuinely unique units remain independent. Only matching Engineers are granted without an earned combat-role equivalent.
...
- Standard mode never generates Foehn units, defenses, or Foehn superpowers. Standard Foehn campaign seeds use Allied and Soviet rewards, matching the production used throughout that campaign. Equivalent Allied/Soviet access is bundled into one reward (for example GI plus Conscript), and compatible buffs are shared across that pair; the Unlocks tab presents both cameos together. Standard All Campaigns uses Allied, Soviet, and Epsilon rewards. The complete Foehn reward catalogue is exclusive to Chaos.
- `Chaos (Experimental)` always randomizes unit access and locks unearned tech. It draws independent access and buff rewards from all four factions regardless of the selected mission campaign. Earned items receive the active player countries plus Ares alternative prerequisite lists for every matching Allied, Soviet, Epsilon, and Foehn production building. Thus an Epsilon/Foehn mission that hands the player an Allied or Soviet base—and a Soviet mission that captures Allied production—shows the same earned infantry, vehicles, aircraft, naval units, and defenses without granting any extra faction production structures. Map-local Phobos `CameoPriority` bands keep that sidebar in contiguous faction blocks: the current player's faction first, then the remaining factions in Allies/Soviets/Epsilon/Foehn order, while Mental Omega's normal order is retained inside each block. By default buffs stay attached only to their named unit. The optional `Share buffs with same-tier equivalent units` setting applies each unit buff to its curated cross-faction role peers as well; the Unlocks tab groups all affected cameos side by side, places their names together on the next line, and lists their combined shared bonuses underneath. Cost and speed are injected directly on affected units; armor becomes unit-specific effective durability. Unsupported unit-specific production-speed rewards are omitted, while the explicitly global Faction Production and Army-wide Fire Rate rewards remain global.
...
- The `Include defensive building rewards` setting controls both defense access rewards and defense-targeted buffs. When enabled, coverage includes 11 Allied, 11 Soviet, 9 Epsilon, and 12 Foehn defenses, from basic anti-ground/anti-air towers through advanced and subfaction defenses. Defense rewards use the Construction Yard as their access prerequisite; defense buffs omit movement-speed upgrades and safely support construction time, cost, armor, health, vision, guarded weapon tuning, repair, cloak, sensors, attack range, and auto-engagement range where applicable. Veteran rewards are generated only for the 20 combat defenses that Mental Omega explicitly marks `Trainable=yes`, and use Ares' `VeteranBuildings` country flag.
...
- Every house-scoped bonus applies to the player and enabled allied helpers through private map-local country copies. These rules cover production, cost, speed, armor, army-wide fire rate, and veterancy. Unit-specific and weapon-specific values are global engine types, so they are applied only when the affected unit or weapon is not also used by an enemy; this avoids buffing enemies and avoids the severe runtime cost of registering complete combat-type variants in campaign maps. Scripted `All change House` handovers to the player are recognized as player forces (for example the Pacific and Chinese forces in Foehn Mission 4), while debug-only handovers are ignored.
...
Archipelago support is intentionally not active yet. The YAML file is only preparation for a future AP world/options layer. Every player-facing generation choice is persisted there, including reward mode, unit access, defensive buildings, buffs, enabled buff types, same-tier Chaos sharing, allied-helper buffs, and superweapons. The current randomizer continues to work fully offline.
