...
| Rewards only when mission is finished | `rewards_on_victory_only` | `false`/`true`; default `false` | Keeps objective tracking but assigns zero player rewards to objectives. The spinner becomes **Rewards per mission** and Mission Victory grants exactly that value multiplied once by mission weight. Objective count no longer changes the mission total. | Seed generation |
