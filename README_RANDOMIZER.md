...
| EVA voice | `eva_voice` | `Mission default`, `Allied`, `Soviet`, `Epsilon`, `Foehn`, `Random`; default `Mission default` | Selects one announcer for the complete launched mission. `Random` deterministically selects one voice from the seed and mission, so it never changes during play. | Every launch |
...
| Start with basic Tier 1 defensive structures | `generation.start_with_tier_one_defenses` | `false` | Grants each available Construction Yard family's basic ground and anti-air defense: Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, and Epsilon Gatling Cannon. Standard follows only physical Allied, Soviet, or Epsilon Construction Yards available to the player; no Yard means no starter-defense cameo. Chaos grants all seven faction defenses behind any available Construction Yard, adding Foehn Sonic Emitter/Shrike Nest. Advanced Pool exclusions override starters. When defensive-building rewards are enabled, their access rewards leave the seed pool while buffs remain eligible immediately. |
...
With **Start with basic Tier 1 defensive structures**, Standard grants Allied Pillbox/Patriot, Soviet Sentry Gun/Flak Cannon, or Epsilon Gatling Cannon only when the matching physical Construction Yard family is available to the player. Every structure keeps its native Construction Yard prerequisite. Missions without player base construction receive no starter defenses. Native Foehn defenses remain Chaos-only. Advanced Pool exclusions always win.
...
The copied aid powers keep installed costs, recharge times, delivered units, and effects unless their profile explicitly corrects a broken dependency. Knightfall keeps its installed `6.5` recharge. The five mine/grid spawners are the timing exception: installed `0.01` is an internal one-shot construction helper, not a usable repeating-power cooldown. Minefields use the reviewed `2.5`-minute player-power timing and Confusion/Stasis grids use `1` minute. Paladin Aid and Knightfall receive their tested targeting and delivery corrections. Drakuv and Harbinger are available only through their aid powers; their `Trainable=no` payload types never appear as production rewards or random tech locks. M.A.D. Mine, Naval Mine, Drakuv, Ruiner, and Kingsnakes remove building/designator, inhibitor, source-range, and shroud gates from their copies while preserving land/water restrictions. Kingsnakes also uses a copied portal object with its separate `PoweredBy` dependency removed. Mercury Strike copies its complete uplink cannon, weapon, projectile, laser, and warhead chain; four invisible map-start uplinks make it independent from `GAMERC`, and its EMPulse cannon targeting range is explicitly `0..9999`. Wallbuster stays the installed `EMPulse` power with its exact `8.5` recharge, cost, 320-damage weapon, projectile, and warhead. Its private weapon, warhead, and projectile are registered in their engine type lists. Four invisible, unbuildable map-start cannon copies use the original turreted `EMPulseCannon=yes` firing path; the Chinese Atomheart never enters the construction tab, copied targeting ranges cover the whole map, and the private weapon accepts the same zero minimum range as the power. Zephyrobot remains an artillery beacon: it needs Zephyr Artillery units already owned by the player and does not create shells by itself. Tactical Nuke stays completely installed/global in every mission except Fatal Impact; there its randomizer copy alone points to a registered private copy of the installed 600-damage `NukePayload`, bypassing the map's 5000-damage objective payload.
