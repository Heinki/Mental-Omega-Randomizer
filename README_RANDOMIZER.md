...
| Include no-build / fixed-unit missions | `generation.include_no_build_missions` | `false`/`true`; default `true` | Includes the 26 reviewed missions played with fixed/scripted units, heroes, or map powers instead of a player-operated production base. Turning it off removes them from the eligible mission pool. See [all 97 mission flags](MISSION_CLASSIFICATION.md). | Seed generation |
| Prioritize no-build missions in opening | `generation.prioritize_no_build_missions` | `false`/`true`; default `false` | Fills protected Mission List or Grid opening positions with the lowest-stage eligible no-build missions first. Ignored by Classic ordering. | Seed generation |
