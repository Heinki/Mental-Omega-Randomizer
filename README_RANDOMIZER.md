...
- Seed-generation settings are copied into `randomizer_state.json`. Changing gameplay settings afterward affects the next seed. Dark mode and reward-name privacy apply immediately.
...
| Hide reward names in Mission Details | `hide_reward_details` | `false`/`true`; default `false` | Replaces assigned reward names with `?????` in Mission Details and mission-row hover text. Earned rewards remain visible in Unlocks. | Immediate |
...
| Start with basic Tier 1 combat units | `generation.start_with_tier_one_units` | `false` | Grants ground and anti-air infantry, ground and anti-air vehicles, plus a basic aircraft from seed start. Standard translates the five roles to each physical Allied, Soviet, or Epsilon production family present in the mission. A player-controlled/scripted MCV or Construction Yard also unlocks that family's airfield. Foehn Standard uses Allied/Soviet operating technology. Chaos keeps one unit from every faction across the four ground roles and adds one seeded Allied/Soviet/Epsilon AircraftType with its native airfield whenever base construction exists. Starter access rewards are removed from that seed's reward pool, but their buff rewards remain eligible immediately. |
...
The **Settings** tab is a vertically scrollable panel. Its scrollbar and mouse-wheel handling keep every reward-pool, buff-type, appearance, and privacy option reachable when the launcher is used at its minimum window size.
...
With **Start with basic Tier 1 combat units**, Standard immediately grants five roles: basic ground infantry, anti-air infantry, a basic ground vehicle, an anti-air vehicle, and a basic aircraft. Their faction follows the physical production family discovered in the map, matching the existing equivalent-access behavior. A discovered player MCV/Construction Yard prepares the corresponding Allied, Soviet, or Epsilon airfield even when the mission did not originally expose it; missions with no player base construction receive no new airfield. For example, Epsilon 04 gives Allied starters after its Allied Construction Yard is captured; a map with Epsilon production gives Epsilon starters even when its campaign or current player house differs. **All Campaigns** supports all Allied, Soviet, and Epsilon production families. Foehn Standard still uses Allied/Soviet operating technology.
...
| Aid/reinforcement | Airborne; Bloodhounds; Zephyrobot; Lightning Rod; Ultra Miner; Kingsnakes; Paladin Aid | Repair Drone; Tank Drop; Instant Shelter; Motor Ambush; Naval Mine; Terror Drop; Flame Tower; Drakuv Prison Vehicle; Repair Drones; Disruptor | Risen Monolith; Scout Raven; Vision; Magnetic Beam; Libra Clones; Bloatick Trap; Quick Fort; Ruiner; Hijackers | Spinblade; Megaarena; Knightfall; Harbinger; Sweeper Drop; Signal Jammer; Decoy Team; Decoy Squadron; M.A.D. Mine |
...
The support/aid pool contains 72 active powers:
...
| Soviets | Repair Drone, Tank Drop, Instant Shelter, Motor Ambush, Naval Mine, Terror Drop, Flame Tower, Drakuv, Repair Drones, Disruptor, Spy Plane, Smoke Bombs, EM Pulse, Irradiation Gamma, Overcharge, Wallbuster, Irradiation Beta, Rad Attack, Pack Attack, EMP Minefield |
...
Source-object-only powers stay excluded because their cloned cameo would be inert without the original object: Blasticade, Golden Wind, Hunter-Seeker, Nanocharge, Gear Change, Psychic Flash, Blackout Missile, Nuclear Path, and Backwarp. Elite Reserves remains excluded because building-free action `34` crashes while creating its internal marker.
...
- Open **Unlocks** to see rewards already earned, grouped by faction/unit with installed cameo art and accumulated buff effects. Offensive, secondary, and support/aid powers display their installed sidebar icon when one exists.
