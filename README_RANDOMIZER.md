...
| Buff allied helpers | `generation.buff_allied_helpers` | `false`/`true`; default `false` | Gives reviewed allied AI helper houses the player's safe country buffs and compatible direct-buff/unit-unlock clones. Existing timing, scripts, and triggers remain intact; compatible helper TaskForce slots and placements use the same buffed `MORP...` clones as the player. Native unit IDs simultaneously retain factory access as invisible-to-player fallbacks for dynamic AI requests outside map TaskForces, preventing the former dead queue. Bounded parallel Autocreate teams add same-faction unlocked clones and never retain capped or unbuildable members. When this option is off, helpers keep only native unbuffed ownership, TaskForces, and veterancy. | Seed generation |
...
Direct unit and weapon definitions are global to the map. The launcher creates narrow standalone `MORP...` unit and `MORW...` weapon copies when needed to isolate an earned buff from enemies. With helper buffs enabled, compatible helper placements and TaskForce consumers use those buffed clones; native originals remain buildable only as dynamic-AI fallbacks. Added Autocreate teams contain only verified level-1 clones. Helper veteran lists prioritize every clone their teams actually produce before fallback IDs so the engine's 480-byte value limit cannot silently remove their veterancy. Positive ownership prevents enemy buff leakage and duplicate player cameos. Installed positive live-unit limits are retained: heroes and super-units remain capped at their normal count and become rebuildable only after one dies; launcher locks `0` and one-build-only `-1` are not copied. Effects that cannot be isolated safely remain skipped and logged. Saved Standard rewards are canonicalized and faction-filtered again at launch, so corrected catalogue entries cannot keep leaking foreign technology from an older seed.
