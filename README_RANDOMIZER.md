...
- **Shop Mode** uses a separate 10-stage roguelike run, three mission choices per stage, Run Ore purchases, and Permanent Gem unlocks. Its progress is stored separately from ordinary Randomizer seeds.
...
|---|---|---:|---|
| Randomize unit access and lock unearned tech | `generation.randomize_unit_access` | `true` | Adds unit access rewards and removes unearned combat technology from player production. Each faction's MCV is an access reward; miners, Engineers, and amphibious transports remain available. Existing map/script-provided MCVs are unaffected. Chaos forces this on. |
...
| Advanced Pool: Units / Buildings | `generation.excluded_unit_access_ids` | List of TechnoType IDs; default empty | Clicking an in-game cameo greys it out and removes that unit, defense, MCV, or special building's access reward **and its unit-specific buff rewards** from future generated seeds. Miners, Engineers, and amphibious transports remain always available. The visible cards follow the selected campaign faction. Existing runs keep their generated reward plan. |
...
`Stage 1 / 10`. A **mission** is one of the maps offered at that stage.
Victories award temporary Run Ore and Permanent Gems. Run Ore buys current-run
units, defenses, powers, and buffs; Permanent Gems buy permanent unit access,
permanent buffs, and account upgrades.
...
Shop Mode opens its own **Shop Mode** workspace. Selecting it replaces the normal Settings page with **Shop Setup** and hides the Advanced tab. Shop Setup shows Seed, the two no-build mission inclusion options, Shop-only Faction Pool, and the Progression selector used to leave Shop Mode. Standard and Chaos reward modes do not apply. The faction pool limits Shop inventory while missions remain mixed-campaign, allowing a ten-stage Foehn-only inventory run. Faction Pool and mission-pool options lock after a run starts. Mission Difficulty Assist is chosen per mission choice during the run. Change game speed in-game; the launcher retains `-SPEEDCONTROL` but no longer exposes an ineffective speed setting.

Start a run from **Shop Setup**, click owned permanent-unit rows to mark the next-run loadout, optionally enable challenge modifiers, then choose from the deterministic mission choices. Selected rows show a check and distinct color; the counter reports used slots. Each seed normally fixes seven mandatory Tier 1 roles—two infantry, two tanks, one aircraft, one naval attacker, and one naval anti-air unit—plus one defense per defense role. One multi-role Epsilon Piranha can satisfy both naval roles. **Elite Force** seed-selects one infantry and one tank to remove while retaining the aircraft, both naval safety roles, and at least one ground anti-air starter. Subfaction variants are candidates for those slots, not extra roster entries. Each mission card contains its own reroll action, which replaces only that choice. Cards also show their seeded game difficulty: stages 1–3 are 80% Casual / 20% Normal, stages 4–5 are 35% Casual / 65% Normal, stages 6–7 are 20% Casual / 60% Normal / 20% Mental, and stages 8–10 are 10% Casual / 45% Normal / 45% Mental. Mission Difficulty Assist lowers the chosen mission by one step while retaining its original Run Ore and Permanent Gem reward.

The first two Shop stages offer Act 1 mission choices, including a fixed-unit or hero mission when available. From stage 3 onward, every remaining eligible mission has equal selection probability: all three choices can be finales. Completed missions cannot repeat, and the selected mission-pool inclusion options still apply.

The header shows stage, Run Status, Run Ore, Permanent Gems, and Mission Rerolls Left. On narrow displays, the details panel collapses when the Shop tab opens, mission cards stack vertically, and the Shop workspace scrolls vertically. Mission-card text wraps within each card; Shop tables retain their own scrollbars for wide columns. **Current Run Shop** sells access and buffs for Run Ore; locked buffs identify their required unit or power and cannot be purchased early. Unit, power, and targeted-buff prices are balanced per target rather than inferred from tier or reward category, so utility rewards can cost less than stronger rewards with similar progression metadata. Purchases persist immediately. **Permanent Unlocks** sells units, account upgrades, and permanent unit-buff stacks for Permanent Gems between runs. Permanent buffs are snapshotted when their unit enters a starting loadout. Starting Run Ore can be raised from 5 to 50. Harder mission classes grant strictly more Permanent Gems: Act 1, Act 2, Operation, then Finale.

Run setup offers 23 persisted modifiers: Greedy, Veteran Economy, Poor Logistics, Generous Command, Blind Choice, Glass Cannon, Overclocked Factories, Black Market, Elite Force, No Safety Net, Support Doctrine, War Economy, Narrow Intelligence, Liquid Assets, Treasure Hunter, Low-Tech War, Superweapon Arms Race, Hardcore, Blockbuster Special, Faction Roulette, Demolition Charges, Melee Fighters, and One Shot, One Kill. Every distinct enabled modifier adds `+1` to the displayed Run Difficulty and adds 1 Run Ore plus 1 Permanent Gem to each mission victory, on top of its own configured reward effects and the existing completion bonus. Percentage effects compose multiplicatively, flat effects add, and all benefit/penalty hooks remain active together. Hardcore forces Mental difficulty and an enemy mission challenge, plus one separate seeded AI buff per mission from the available armor, production, health, firepower, fire-rate, and mobility buffs. The extra buff appears separately in enemy scaling details. Missions with protected scripted openings or no-build enemy-buff safeguards retain those safety exceptions. Narrow Intelligence reduces the visible mission choices to two. No Safety Net disables Mission Rerolls, Mission Difficulty Assists, and Emergency Revivals at both UI and transaction boundaries. Demolition Charges makes all mobile player and AI units explode on death. Melee Fighters limits mobile-unit and base-defense weapons to 2.35 cells with no minimum range. One Shot, One Kill gives every registered unit and building 1 health. Active-run modifier controls lock until the run ends.

The Current Run Shop presents the rotating unit, building, aid-power, and superweapon stock for the current stage in one list. Access already active from starters, permanent/AP loadouts, or run purchases is excluded and replaced, so an owned unit or structure never wastes a stock slot. The **Available / Owned** filter switches between current stock and every active access item. Search covers reward and target IDs; sorting supports name, tier, price, or status. Buy an access item, then use its **Open Upgrades** action to see only its valid buffs. Buff rows remain selected after a stack purchase for rapid repeat buying. Unit targets are chosen from searchable tables instead of unbounded dropdowns. Mission and catalogue tooltips show exact reward/price state. Victory messages show base reward, modifier adjustment, permanent Victory Bonus, and total.

Each Shop mission card has scrollable details above its fixed launch and reroll buttons. It lists each enemy AI effect with its final combined value, without stack totals or source labels. It also shows the exact Ore and Gem change from each run modifier and the mission challenge separately. Protected missions show why no enemy buffs are assigned.
...
Permanent Units has explicit **All**, **Not Owned**, and **Owned** filters. Permanent unit buffs use a guided flow: buy a permanent unit between runs, select it on **Permanent Unlocks > Units**, then open **View Buffs for Owned Unit** and purchase stacks. Free Buff Tokens grant player-chosen Current Run Shop buffs; the redundant random Starting Buff Draft is retired for new runs. Existing profiles and active runs containing its saved fields remain readable. The saved `discount_specialization` upgrade now reduces every Current Run Shop unit, buff, and power price; no category selector is required. **Reset Profile…** permanently clears Permanent Gems, permanent purchases, lifetime totals, and the current run only after an explicit warning.

Five further permanent upgrades are available: **Coupon Book** discounts the first paid purchase of each stage; **Stock Lock** preserves one selected access stock entry into the next stage; **Veteran Academy** makes selected permanent-loadout units Veteran; **Gem Dividend** grants 1 bonus Permanent Gem per 5 Run Ore held before each mission victory, capped at 2 Permanent Gems per upgrade level without spending Run Ore; and **Premium Supplier** guarantees a later-stage higher-tier access stock entry. These upgrades add progress without penalties.

**Run Summary** preserves active, failed, and completed results across restarts. It reports seed, stages cleared, remaining Run Ore, Permanent Gems, purchases, buff stacks, modifiers, missions won, and the failed mission/stage when applicable. Completion opens a `RUN VICTORY` summary; failure opens `RUN OVER` while preserving permanent progression.

Launching commits the selected mission choice before map preparation. Other choices, rerolls, and purchases lock until the mission ends; a launcher or game crash before a detected result can only relaunch that committed mission. Shop starters, selected permanent units, run purchases, and purchased buff stacks use the normal isolated player-clone map pipeline. Victory grants the displayed currencies and creates the next three mission choices. Stage 10 victory completes the run.

For newly generated Archipelago Shop slots, every received AP unit unlock is active in every run without consuming permanent extra-unit slots. Received buffs and powers are reapplied automatically, including legitimate stacks from distinct received item indexes. The run stores a snapshot keyed to the validated room, team, and slot; new items received during an active run are merged immediately and become available under their normal rules. Failure never copies, spends, or removes AP items. Starting another run rebuilds the snapshot from the existing AP received-item ledger, including while disconnected, and replayed item indexes do not grant twice. This AP restart behavior does not read or change mission credits, Run Ore, or Permanent Gem balances.

Generated Archipelago Shop seeds contain 120 shuffled item locations and add an **AP Purchases** panel. Each entry uses the Archipelago emblem and shows the server-scouted item name plus its recipient player/world, making cross-world purchases distinct from Mental Omega unit cameos. Each available entry costs the Permanent Gem amount frozen into the room and reports one generated location; the server, never the launcher purchase button, determines which item it sends. The Permanent Gem debit is saved before the check is queued, then reconciled with server checked-location state after reconnecting, so a retry cannot charge twice. Received AP rewards use the same emblem in Shop loadouts and both Unlocks views. Every Shop stage victory releases up to 12 previously unchecked item locations, including across failed and restarted runs. Completing stage 10 releases every remaining item location before sending the Shop run goal.

A detected defeat, in-game restart, or game-process exit without a victory marker ends the run. Current game logging cannot reliably distinguish every manual quit or crash from defeat, so those exits use the existing launcher failure semantics and also end the run. Permanent Gems, units, and upgrades survive; Run Ore and run purchases do not carry into the next run.
