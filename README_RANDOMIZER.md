...
- Adds an `Unlocks` tab that groups earned access rewards and buffs by faction and displays each unit's real in-game cameo. Shared Chaos buff groups place their faction-ordered cameos side by side with names and combined bonuses. Cameo PCX files are extracted from the installed MIX archives, decoded without an external Python image package, and cached under `RandomizerLauncherData\cameo_cache` in packaged mode.
