...
- color choice for player (low prio)
- Remove limit for heroes as a bonus (being able to build two tanyas e.g.)
- Bad apple crashing in the middle of the mission
- special building unlocks for their corresponding faction and in chaos mode
...
- Double units unlocks like humvee or mines
- Make robot tanks for allies available, for some reason it is not there for navalyard and war factory
- Army wide fire rate might not be working, need to double check
- burrilo not working when used in warfactory, seemed to be only the case when the unit was not unlocked
- chaos mode, cloaking given with buffing the same type. GI was cloaked BUT the Conscript was not. - might be because of enemy tech skip
- defenses self heal does not work
- Hide mission unlocks in the UI, for those player who like suprises
- Scrolling on big Grid mode as you cannot see the other missions on the board
- Add dark theme
- Add custom selection of maps into the pool
- When a mission was started but not finished the progress text appears should be removed to let the Player know which mission is still needed to complete
-  Check updater for randomizer
-Air units not useable when no airfield
...
New Game mode idea:
The levels will be divided into 3 categories:
